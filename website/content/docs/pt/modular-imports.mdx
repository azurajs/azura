...
<Callout type="warn">
  **Atenção!** A partir da versão 2.4.0, você DEVE usar imports modulares. O pacote principal (`azurajs`) exporta apenas `AzuraClient`, `applyDecorators` e tipos básicos. Funcionalidades específicas como logger, validators, http-error, cors, etc. devem ser importados de seus respectivos módulos.
</Callout>
